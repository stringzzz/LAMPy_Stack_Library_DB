...
def mysql_run_query(inputs):
	...
	try:
		...
		my_db = mysql.connector.connect(
			user='Librarian',
			password='********', #Set to password you created for this user
			host='localhost',
			database='library_db'
		)
		...
		query_string = ""
		if no_filters:
			query_string = f"SELECT {", ".join(inputs['output_columns'])} FROM Books;"
		else:
			query_string = f"SELECT {", ".join(inputs['output_columns'])} FROM Books WHERE {" AND ".join(filters)};"
